\documentclass[12pt,french]{report}% Mise en page d’un rapport de plusieurs chapitres
\input{tex/package}
\input{tex/macro}
\input{tex/page-titre}
\input{tex/acronyme}
\makeindex
\begin{document}
\graphicspath{{image/}}
%\layout
\maketitle
\input{tex/compteur}
\input{tex/en-tete}
\tableofcontents
\listoftables
\listoffigures
\input{chapitre/00}
\input{chapitre/01}% Préliminaires
\input{chapitre/02}% Revenus et dépenses d'emploi
\input{chapitre/03}% Déductions et crédits d'emploi
\input{chapitre/04}% Montants personnels
\input{chapitre/05}% Calcul des impôts et crédits
\input{chapitre/06}% Revenus de placement
%\input{chapitre/07}% Prestations sociales
%\input{chapitre/08}% Régimes enregistrés
%\input{chapitre/09}% Santé
%\input{chapitre/10}% Les contribuables aînés
%\input{chapitre/11}% Situation familiale
%\input{chapitre/12}% Les étudiants
%\input{chapitre/13}% Renseignements administratifs
\input{tex/glossaires}
\input{tex/formulaire-fed}
\input{tex/formulaire-qc}
\input{tex/index}
\end{document}

% ‘
% ’
% «
% »
%  ?
%  ;
%  :
% ; et
% Aperçu - Fichier - Imprimer - PDF (gauche) - Enregistrer au format PDF
